...
object SparkUserModelStackingMain {
  ...
  def train_xgboost(sparkSession: SparkSession, params: Params): Unit = {
    ...
    var count = 1
    while(startDate < params.initDate && count <= 10){
      val (trainDF, testDF) = DataLoadUtils.loadUserTrainData(sparkSession, params.inputPath+s"_part$count", params.fracTest)
      // start training
      val paramMap = List(
        "eta" -> 0.05f,
        "max_depth" -> 8,
  ...
  def predict(sparkSession: SparkSession, params: Params): Unit = {
    ...
    val script_sql = AlgoUtils.genModelBlendFeatureSql("user_model_blend_pred_feature.sql")
...
//    }
  ...
  def train_xgboost_stacking(sparkSession: SparkSession, params: Params): Unit = {
    ...
    val script_sql = AlgoUtils.genModelBlendFeatureSql("user_model_blend_feature.sql")
    val df_blend_feature = sparkSession.sql(script_sql)
  ...
  def predict_eval(sparkSession: SparkSession, params: Params): Unit = {
    ...
    val lr_result = cvModel.transform(dataSets).select("user_id", "label","probability","prediction")
    val evaluator = new MulticlassClassificationEvaluator()
      .setLabelCol("label")
      .setPredictionCol("prediction")
